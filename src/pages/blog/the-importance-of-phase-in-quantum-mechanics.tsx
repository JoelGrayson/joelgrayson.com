...
export default function TheImportanceOfPhaseInQuantumMechanics() {
    return <ArticleWrapper title='The Importance of Phase in Quantum Mechanics' date={new Date('Jan 21, 2023')} hyphenatedTitle='the-importance-of-phase-in-quantum-mechanics'>
        <P>Although when finally measuring, the phase does not play an impact on the probability of measuring a state, it does play an impact in the quantum circuit before measurement when quantum gates are applied. While an entangled state is going through a quantum circuit, its phase is affected by its interactions with the gates and can lead to different probabilities. For example, {<LaTeX number={1} />} and { <LaTeX number={2} />} when measured have the same 50–50 probability of measuring {<LaTeX number={3} />} or {<LaTeX number={4} />} (the phase does not impact measurement), but in {<LaTeX number={5} />} and {<LaTeX number={6} />}, the phase does impact the final measurement.</P>
